import torch
import numpy as np
from learning.gail_old.utils.utils import get_entropy, log_prob_density
...
def train_actor_critic(actor, critic, memory, actor_optim, critic_optim, args):
    states = np.vstack([entry[0] for entry in memory])  # Stack states vertically
    actions = np.array([entry[1] for entry in memory])  # Convert actions to array
    rewards = np.array([entry[2] for entry in memory])  # Convert rewards to array
    masks = np.array([entry[3] for entry in memory])    # Convert masks to array	    	
