...
from utils.mj import attach, get_mj_data, get_mj_model, body_name2id
...
class PickAndPlaceSim:
    ...
    def keyboard_callback(self, key) -> None:
        ...
        if key == glfw.KEY_SPACE:
            ...
            point_translated = self.get_box_position()
            current_ee_pose.t = point_translated

            box_position = self.get_box_position()

            robot_pose = self.ur5e.get_ee_pose()

            #
            robot_pose.t = box_position

            joint_vals = self.ur5e.ik(robot_pose)
